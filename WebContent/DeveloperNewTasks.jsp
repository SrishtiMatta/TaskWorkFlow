...
function ConfirmAssign(){
	var r = confirm("Are you sure you want to take up this task");
	if(r==true){
		return true; 
	}
	else
...
<% 	if(session.getAttribute("USER_DETAILS")==null){
		...
		<%	Connection connection = null;
	        ...
	        PreparedStatement ps=(PreparedStatement) connection.prepareStatement("select * from tasks where TargetRole=? && Status=?");
    	...
    	%>
      		...
      		<p style="text-align: center; color: yellow; font-size:20px "><%out.print("You have no New Tasks right now");%></p> 
      <%
	        }else{
      %>
       ...
       <TABLE BORDER="1">
    		...
    		%>
           		<TR>
	                ...
	                <TD><a href="AssignTask?TaskID=<%=resultset.getString(1) %>&TaskName=<%= resultset.getString(2)%>&UserId=<%= resultset.getString(4)%>"  onClick ="return ConfirmAssign();" >Assign</a></TD>
	            </TR>
			<% 
